...
$customers = $pdoStatement->fetchAll(); // 3. Récupération les résultats de la requête
...
$sql = 'SELECT *
        FROM customers
        WHERE customerNumber = ?'; // ? : caractère joker, il peut y en avoir plusieurs = ?, ?, ?'

/** @var PDOStatement $pdoStatement */
$pdoStatement = $pdo->prepare($sql); // 1. préparation de la requête
$pdoStatement->execute([$customerNumber]); // 2. Exécution de la requête - [$customerNumber] = ?
